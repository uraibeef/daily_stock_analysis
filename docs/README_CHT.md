...
| 多市場數據聚合 | A股、港股、美股、ETF：行情、K 線、技術指標、資金流、籌碼、新聞、公告和基本面；日股/韓股/台股（`.T` / `.KS` / `.KQ` / `.TW` / `.TWO`）：支援 YFinance 日線、基礎行情和技術指標，台股額外接入三大法人報告區塊、TWD 幣種和收盤競價階段識別；A 股專屬資金流、龍虎榜、板塊等高階區塊會依市場邊界降級為 `not_supported`（見 [市場支持邊界](market-support.md)） |
