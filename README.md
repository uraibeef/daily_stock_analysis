...
| 多市场数据聚合 | A股、港股、美股、ETF：行情、K 线、技术指标、资金流、筹码、新闻、公告和基本面；日股/韩股/台股（`.T` / `.KS` / `.KQ` / `.TW` / `.TWO`）：YFinance 日线、基础行情和技术指标可用，台股额外接入三大法人报告区块、TWD 币种和收盘竞价阶段识别；A 股专属资金流、龙虎榜、板块等高阶区块会按市场边界降级为 `not_supported`（见 [市场支持边界](docs/market-support.md)） |
